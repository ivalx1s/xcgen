...
""",
    ...
    func run() throws {
        ...
        guard fileManager.fileExists(atPath: fileURL.path, isDirectory: &isDirectory) else {
            throw ValidationError("Couldn't find a JSON manifest at \(fileURL.path). Pass the path to the file generated by your XcodeGen tooling (e.g. debug_xcodegen_project_spec.json).")
        }
        if isDirectory.boolValue {
            let ext = fileURL.pathExtension.lowercased()
            let hint = (ext == "xcodeproj" || ext == "xcworkspace")
                ? " It looks like you passed an .\(ext) bundle; xcgen fetch expects the manifest JSON instead."
                : ""
            throw ValidationError("\(fileURL.path) is a directory.\(hint)")
        }
        ...
        do {
            ...
            throw ValidationError("Unable to read manifest at \(fileURL.path): \(error.localizedDescription)")
        }
        ...
        ManifestUpdater.addLocalPackages(to: fileURL,
                                         repositoryNames: checkedOutPackages)
    }
...
struct GenerateProject: ParsableCommand {
    static let configuration = CommandConfiguration(
        ...
        abstract: "Generate an Xcode project by invoking XcodeGen.",
        shouldDisplay: false
    )
